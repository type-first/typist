...
### A Minimal, Precise, Type‑Level Utility Suite for TypeScript

Typist provides a set of rigorously focused primitives for **compile‑time verification**, **type introspection**, and **structural analysis**. Every export is intentionally small in scope yet compositionally powerful, enabling developers to write, test, and reason about complex type systems with clarity and control.

Typist is designed for teams and individuals who treat the TypeScript compiler as part of their architecture: a static engine for enforcing invariants, describing domain models, verifying assumptions, and accelerating code generation through precise inference.
...
Typist is designed to support and encourage a **type‑first approach**: leveraging the compiler as an active analytical tool, enforcing architectural constraints, validating domain models, and enabling precise, inference‑driven workflows. This style of development can significantly strengthen code generation, refactoring reliability, and static correctness.

If you want a broader picture of the ideas behind this style of work, see:

* **The Type‑First Revolution** (`/guides/type-first-revolution`)
* **Principles of Type‑First Development** (`/guides/type-first`)
* **What Typist Solves** (`/guides/typist-concepts`)

For applied examples and case studies:

* **Phone Number Types (Case Study)** (`/articles/phone-numbers`)
* **Architectural Constraints via Types** (`/articles/architecture`)

For live, compilable demonstrations:

* **Typescapes** (`/examples/*`)

All exports end in `_` to indicate that they operate in the type‑first space.

---

## 1. Assertions

*For extended documentation and deeper examples, see: `/docs/assertions` on the website.*
...
const p2 = { name: "B", age: 30 };

// T is inferred as { name: string; age: number }
const person_ = like_(p1, p2);

// you can now use typeof person_ as the shared structural type
is_<{ name: string; age: number }>(person_);

// using different but compatible shapes
const p3 = { id: 1, name: "A", age: 20 };
const p4 = { id: 2, name: "B" };

const commonPerson_ = like_(p3, p4);
