...
* Recursive and conditional type inspection with customizable diagnostic type data
* Type materialization utilities for resolving complex inferred types
* Block-based grouping for type-level test suites

For a full API reference guide with extended examples, explanatory guides, and conceptual overview, see full [Documentation](docs/typist).

### Essential Primitives for Type-First Development

Typist exists to support a type‑first approach to software development: leveraging the compiler as an active analytical tool, enforcing architectural constraints, validating domain models, and enabling precise, inference‑driven workflows. This can significantly strengthen code generation capability. To learn more about type-first development, see [The Type‑First Revolution](articles/the-type-first-revolution).
...
See full [Assertions](docs/typist/assertions) documentation.
...
See [Verdicts](docs/typist/verdicts) for design rationale and extended diagnostics examples.
...
A phantom value is a nominal runtime value used solely to **transport type information**. See [The Power of Phantoms](/articles/the-power-of-phantoms) to learn more about phantom values.
